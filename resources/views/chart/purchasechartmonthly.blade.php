...
<div class="card">
    ...
    <div class="card-body">
        <div class="col-sm-12 col-md-12">
            {!! $purchasechartmonthly->container() !!}
        </div>
    </div>


</div>
{!! $purchasechartmonthly->script() !!}
<script>
    $("#viewchart").change(function(){
var type=$(this).val();
if(type==2){
    url = "{{ route('chart.purchaseViewYearly')}}",
      window.location = url;
}
    });
    var invoice_api_url = {{$purchasechartmonthly->id}}_api_url;
    ...
    function ChartFileter() {
        ...
        var month = $(".monthchart").val(); 
         {{$purchasechartmonthly->id}}_refresh(invoice_api_url + "?year=" + year + "&type=" + type+"&month="+month);
    }
</script>
@endsection