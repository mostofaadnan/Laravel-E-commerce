...
<script>
    $("#viewchart").change(function() {
                var type = $(this).val();
                if (type == 1) {
                    url = "{{ route('chart.supplieyPaymentMonthlyView')}}",
                        window.location = url;
                }
            });
            var purchase_api_url = {{ $SupplierPaymentChart->id }}_api_url;
    ...
    {{ $SupplierPaymentChart->id }}_refresh(purchase_api_url + "?year="+year+"&type="+type);
}
</script>
@endsection