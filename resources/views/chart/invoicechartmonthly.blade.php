...
<div class="card">
    ...
    <div class="card-body">
        <div class="col-sm-12 col-md-12">
            {!! $invoicechartmonthly->container() !!}
        </div>
    </div>


</div>
{!! $invoicechartmonthly->script() !!}
<script>
    $("#viewchart").change(function(){
var type=$(this).val();
if(type==2){
    url = "{{ route('chart.invoiceview')}}",
      window.location = url;
}
    });
    var invoice_api_url = {{$invoicechartmonthly->id}}_api_url;
    ...
    function ChartFileter() {
        ...
        var month = $(".monthchart").val(); 
         {{$invoicechartmonthly->id}}_refresh(invoice_api_url + "?year=" + year + "&type=" + type+"&month="+month);
    }
</script>
@endsection